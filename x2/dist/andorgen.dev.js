...
function Argo(objParent, nOpt) {
  ...
  Argo.prototype.ShootSpario = function (nOftX, nOftY) {
    ...
    if( _random(100) > this.m_nFiringPossibility )
    	return null;
    */
    var nX = parseInt(this.m_nodeParent.style.left) + parseInt(this.m_nodeThis.style.left) + this.m_nSize / 2;
    var nY = parseInt(this.m_nodeParent.style.top) + parseInt(this.m_nodeThis.style.top) + this.m_nSize / 2;
  ...
  Argo.prototype.Move = function () {
    if (this.m_nHitCount == 0) {
      ...
      var nOftX = -32 * (this.m_nHitCount / 4 | 0);
  ...
  Argo.prototype.Hit = function (bScore) {
    if (this.m_nHitCount != 0) return;
    this.m_nHitCount = 1;
    if (bScore) g_App.GetScoreObject().AddScore(this.m_nScore);
    this.m_nBKOftX -= 14;
...
function AndorGen() {
  var STATUS_COMING = 0;
  var STATUS_FLOATING = 1;
  var STATUS_GOING = 2;
  var STATUS_DEAD = 3;
  var STATUS_ESCAPING = 4;
  this.base();
  ...
  if (g_App.GetMapObject().GetCurrentArea().GetAreaNo() >= 14 && g_App.GetMapObject().GetCurrentArea().GetScrollPos() < 150) this.m_nLeavingTime = 40;
  this.Create(this.m_objCurArea, (SCREEN_WIDTH - 96) / 2 | 0, this.m_nTop);
  this.m_nodeThis.classList.remove('size16');
  this.m_nodeThis.classList.add('size96');
  this.m_nodeThis.classList.remove('ground-shadow');
  if (g_App.GetConfigObject().IsShadow()) this.m_nodeThis.classList.add('andor-shadow');
  this.m_nodePicture.classList.remove('size16');
  ...
  AndorGen.prototype.HitTest = function (nX, nY) {
    ...
    for (var _i = 0; _i < 4; _i++) {
      if (this.m_objArgoAr[_i].HitTest(nX, nY)) {
        this.m_objHit = this.m_objArgoAr[_i];
        return true;
      }
    }
  ...
  AndorGen.prototype.Hit = function () {
    if (!this.m_objHit) return false;

    if (this.m_objHit == this) {
      if (this.m_nHitCount == 0) {
        this.m_nHitCount = 1; //余分な点数が入るバグの再現

        var nAdditionalScore = 0;

        switch (g_App.GetMapObject().GetCurrentAreaNumber()) {
          case 4:
            nAdditionalScore = 800;
            break;

          case 9:
            nAdditionalScore = 200;
            break;

          case 14:
            nAdditionalScore = 400;
            break;
        }

        if (g_App.GetGameObject().IsRestarted()) nAdditionalScore = 10;
        ...
        for (var _i2 = 0; _i2 < 4; _i2++) {
          this.m_objArgoAr[_i2].Hit(false);
        }
    ...
    for (var _i3 = 0; _i3 < 4; _i3++) {
      if (this.m_objArgoAr[_i3] == this.m_objHit) {
        this.m_objArgoAr[_i3].Hit(true);
  ...
  AndorGen.prototype.Move = function () {
    ...
    if (this.m_nStatus == STATUS_COMING) {
      ...
      var nCurPos = this.m_objCurArea.GetScrollPos();
      ...
      if (this.m_nHitCount > 0) {
        ...
        this.m_nodeExplosion.style.backgroundPosition = nOftX + 'px 0px';
        this.m_nHitCount++;
        if (this.m_nHitCount == 16) this.m_nodePicture.style.backgroundPosition = '-288px 0px';
      ...
      if (nBottom > SCREEN_HEIGHT + 96) return false;
    }

    this.m_nTop += this.m_nSpeed;
    this.m_nodeThis.style.top = Math.round(this.m_nTop) + 'px'; //砲台の攻撃

    for (var _i4 = 0; _i4 < 4; _i4++) {
      this.m_objArgoAr[_i4].Move();
    }
...
function Bragza(nodeAndorGen) {
  ...
  this.Show(true);
  this.m_pos = {
    x: parseInt(this.m_nodeAndorGen.style.left) + 48,
    y: parseInt(this.m_nodeAndorGen.style.top) + 48
  };
  this.m_vec = {
    u: 0,
    v: -1
  ...
  Bragza.prototype.Move = function () {
    if (!FlyingObject.prototype.Move.call(this)) return false;
    var nIndex = (this.m_nCount / 2 | 0) % 8;
    this.m_nodePicture.style.backgroundPosition = -(nIndex + 1) * 16 + 'px ' + this.m_nBKOftY + 'px';
    this.m_nCount++;
    if (this.m_nCount >= 16) this.m_nCount = 0;
    return true;
  };
}
//# sourceMappingURL=andorgen.dev.js.map
